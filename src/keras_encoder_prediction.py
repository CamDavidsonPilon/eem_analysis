...
from src.utils import FOLDER, load_images
...
for _id in np.arange(0, 200, 40):
    fig = plt.figure(figsize=(6, 8))
    axes = fig.subplots(3, 1)

    axes[0].imshow(images[_id].reshape(28, 28))
    axes[0].set_title("Actual")

    axes[1].imshow(prediction[_id][None, :], cmap=cm.Greys)
    axes[1].set_title("Latent representation")

    axes[2].imshow(prediction_images[_id].reshape(28, 28))
